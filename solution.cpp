...
std::vector<int> compress_row(const std::vector<int>& row) {
	...
	copy_if(row.begin(), row.end(), back_inserter(compressed), not_zero);
...
bool move_left(vector<vector<int>>& board) {
	...
	for (int i = 0; i < 4; i++) {
		vector<int> new_row = merge_row(board[i]);
		if (new_row != board[i]) {
			change = true;
		}
		board[i] = new_row;
...
bool move_up(vector<vector<int>>& board){
	...
	for (int i = 0; i < 4; i++) {
		...
		vector<int> new_col = merge_row(column);
...
bool move_down(vector<vector<int>>& board){
	vector<vector<int>> original = board;
        ...
        for (int i = 0; i < 4; i++) {
                ...
                for (int j = 3; j >= 0; j--) {
                        if (board[j][i] != new_col[3 - j]) {
                                change = true;
                        }
                        board[j][i] = new_col[3 - j];
                }
        }
        return change;
}
